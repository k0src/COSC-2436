...
void roundRobinScheduling(Process processes[], int n, int time_quantum) {
    ...
    while (!q.empty()) {
        ...
        // Execute the process for either its remaining burst time or time quantum
        int execution_time = std::min(current.burst_time, time_quantum);
        std::cout << "Executing Process P" << current.id << " for " << execution_time << " units" << std::endl;
        ...
        if (current.burst_time > 0) {
            ...
            std::cout << "Process P" << current.id << " completed at time " << total_time << std::endl;
        }
    }
...
int main() {
    Process processes[] = {{1, 10}, {2, 4}, {3, 6}};
